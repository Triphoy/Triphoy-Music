...
const Upload = () => {
  ...
  if (!file.type.includes('audio/')) {
    setError('Пожалуйста, загрузите аудиофайл (MP3, WAV, и т.п.)');
    return;
  }

  if (file.size > 10 * 1024 * 1024) {
    setError('Максимальный размер трека — 10MB. Пожалуйста, сожми файл.');
    return;
  }
  ...
  const handleCoverChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.match('image.*')) {
      setError('Пожалуйста загрузите обложку в этих форматах (PNG, JPEG)');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="upload-form">
          <div className="file-upload-section">
            <div className="file-upload-group">
              <h3>Аудио Файл *</h3>
              <p>Поддерживаются эти форматы: MP3 (max 10MB)</p>
              <input
                type="file"
                ref={audioInputRef}
